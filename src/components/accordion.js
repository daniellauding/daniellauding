  ...
  return (
    <div>
      ...
      {accordionItem.map((item, index) => (
        <div key={index} className={classNames(``, item.active ? 'active' : item.active)}>
          <div>
            <button type="button" role="tab" tabIndex="0" onClick={() => setActiveAccordion(index)}>
              <h3 className="pt-0 mt-8 mb-0 text-2xl text-center dark:text-white text-black lg:font-bold">
                {item.title}
              </h3>
            </button>
          </div>
          <div
            className={classNames('accordion-item', `accordion-item-${index}`, {
