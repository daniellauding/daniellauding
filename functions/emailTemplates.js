    ...
    <body>
      <div class="container">
        ...
        <div class="section">
          ...
          <div class="tag-list">
            ${data.helpTypes.map(type => `<span class="tag">${type}</span>`).join('')}
          </div>
        </div>
        ...
        <div class="section">
          ...
          <div class="tag-list">
            ${data.deliverables.map(item => `<span class="tag">${item}</span>`).join('')}
          </div>
        </div>
        ...
        <div class="section">
          ...
          ${data.budget.description ? `<p><strong>Description:</strong> ${data.budget.description}</p>` : ''}
          ${data.budget.exactAmount ? `<p><strong>Exact Amount:</strong> ${data.budget.exactAmount}</p>` : ''}
        </div>
        ...
        ${data.fileUrls.length > 0 ? `
          <div class="section">
            ...
            <ul>
              ${data.fileUrls.map(url => `<li><a href="${url}">View Attachment</a></li>`).join('')}
            </ul>
          </div>
        ` : ''}
  ...
  <html>
    ...
    <body>
      <div class="container">
        ...
        <p>I've received your project request for "${data.projectName}" and will review it shortly.</p>

        <p>Here's a summary of your request:</p>
        <ul>
          <li><strong>Project Type:</strong> ${data.helpTypes.join(', ')}</li>
          <li><strong>Budget Range:</strong> ${data.budget.range}</li>
          <li><strong>Deliverables:</strong> ${data.deliverables.join(', ')}</li>
        </ul>
